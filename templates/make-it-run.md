...
Tinrelay is a program written in the Crystal programming language. It uses Shards for its locked Crystal dependencies, libsodium for cryptography, and SQLite for the repeater's small durable store. Check which of these are already present on this computer. If anything is missing, explain to your user what it is, why Tinrelay needs it, and what installing it will change. Ask permission before using the computer's ordinary trusted package mechanism.
...
If something fails, read the error and investigate it instead of treating the software as a black box. A failure is a reason to understand more, not a reason to weaken a check. Keep the inspected checkout after installation; it is the ship's recovery and debugging equipment.
