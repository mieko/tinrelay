...
Adapt `templates/tinrelay-radio-room.md` with the ship name and mapping-file path, then create the task in this home with the exact title `tinrelay-radio-room`. Use Luna High or a model with comparable instruction-following; Luna Light is not reliable enough for the wait-route-mark-wait loop. Choose a model and reasoning combination the task interface actually offers. If task creation rejects it once, stop retrying, show your user the exact failure, and choose a supported combination together. The radio room does not need the model you are using now. Because it shares your home, make sure the guidance keeps its role separate from yours and from any task that appears later.
